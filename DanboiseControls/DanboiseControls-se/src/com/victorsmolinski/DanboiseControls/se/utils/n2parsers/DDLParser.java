...
*/
                    ...
                    try{
                        ...
                        if(line.startsWith("*") || !line.contains(",") ||
                            ...
                            logger.info("DDL PARSER LINE DISCARDED...!"+" ".concat(line));
                            ...
                            if(pointProps.length == 6) {
                                ...
                                for(char t : tokens){
                                    if(Character.isLetter(t)){
                                        ...
                                        logger.warning("DDLParser, type-addr field unknkown token...!");
                                    }
                                }
                                if(_sb.toString().length() > 3){
                                    final String _sub = _sb.toString()
                                            .substring(1);
                                    logger.warning("3-digit-addr-pts: ".concat(_sub));
                                    addr = Integer.parseInt(_sub);
                                } else { addr = Integer.parseInt(_sb.toString()); }
                                logger.info("DDLParser...N2 Point address: "+addr);

                                if(sb.toString().length() > 2) {
                                    final String sub = sb.toString().substring(sb.toString().length() - 3);
                                    subType = sub;
                                } else {subType = sb.toString();}
                                logger.info("N2 POINT SUB-TYPE: "+subType);

                                /**
                                 * DEPRECATED 2021-08-28 IN FAVOR OF THE CODE ABOVE
                                 */
...
//                                logger.info("N2 POINT SUB-TYPE: "+subType);
                            ...
                            logger.warning("DDL PARSER -UNKNOWN SCHEMA, LINE DISCARDED...!"+" ".concat(line));
                        }}catch(Exception e){
                        logger.severe(e.getMessage());
                        e.printStackTrace();
                    }
